...
- **UI** — `MainActivity` (list + permission request + Syslog dialog + "Retry N failed" and Settings action-bar items), `ListAdapter` (renders configs, toggles), `ForwardingConfigDialog` (add/edit form), `SettingsActivity` (global/heartbeat settings). The Syslog viewer shells out to `logcat` via `Runtime.exec` filtered to this package.
